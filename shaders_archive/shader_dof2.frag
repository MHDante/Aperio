/*******************************************************************
	Fragment
	DOF Post-process Shader : Depth of Field Shader
*******************************************************************/
 /**
...
float focalDepth = 1.0;  //focal distance value in meters, but you may use autofocus option below
float focalLength = 24.0; //focal length in mm
float fstop = 0.9; //f-stop value
...
float bdepth(vec2 coords) {
	...
	for( int i=0; i<9; i++ ) {
		float tmp = texture2D(sourceDepth, coords + offset[i]).r;
		d += tmp * kernel[i];
	}
...
void main() {
	...
	float depth = linearize(texture2D(sourceDepth,vUv.xy).x);
	...
	if (autofocus) {
		...
		fDepth = linearize(texture2D(sourceDepth,focusCoords).x);
	...
	if(blur < 0.05) {
		...
		col = texture2D(source, vUv.xy).rgb;
	} else {
		col = texture2D(source, vUv.xy).rgb;
		float s = 1.0;
		int ringsamples;
