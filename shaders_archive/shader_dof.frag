...
float focalDepth = 20.0;  //focal distance value in meters, but you may use autofocus option below
float focalLength = 20.0; //focal length in mm
float fstop = 1.0; //f-stop value
...
float CoC = 0.01;//circle of confusion size in mm (35mm film = 0.03mm)
...
void main() 
{
	...
	float depth = linearize(texture2D(sourceDepth,gl_TexCoord[0].xy).x);
	...
	if (autofocus)
	{
		fDepth = linearize(texture2D(sourceDepth,focus).x);
	}
