...
Module:    vtkMyFillHolesFilter.cxx
...
vtkStandardNewMacro(vtkMyFillHolesFilter);

//------------------------------------------------------------------------
vtkMyFillHolesFilter::vtkMyFillHolesFilter()
{
	this->HoleSize = 1.0;
}
...
//------------------------------------------------------------------------
int vtkMyFillHolesFilter::RequestData(
	...
	vtkInformationVector *outputVector)
{
	...
	vtkPolyData *output = vtkPolyData::SafeDownCast(
		...
		vtkPolyData *newInput = vtkPolyData::New();
		newInput->SetPoints(vtkPoints::New());

		for (int i = 0; i < input->GetNumberOfPoints(); i++)
		{
			double pt[3];
			input->GetPoint(i, pt);
			newInput->GetPoints()->InsertNextPoint(pt);
		}

		newInput->SetPolys(vtkCellArray::New());
		...
		for (int i = 0; i < input->GetPolys()->GetNumberOfCells(); i++)
		{
			...
			newInput->GetPolys()->InsertNextCell(npts, pts);
		}
...
//------------------------------------------------------------------------
void vtkMyFillHolesFilter::PrintSelf(ostream& os, vtkIndent indent)
{
