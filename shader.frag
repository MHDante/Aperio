...
vec3 minnaert(vec3 L, vec3 n, float k) {
...
void phongLighting(int i)
{
	...
	//vec4 Idiff =  ( (gl_FrontMaterial.diffuse * 0.5 + 0.3 * texture2D(mytext, gl_TexCoord[0].st) + 0.0) * gl_LightSource[0].diffuse *
	//vec4 Idiff =  ( (gl_FrontMaterial.diffuse * texture2D(mytext, gl_TexCoord[0].st)) * gl_LightSource[0].diffuse *
	...
	float roughness = 1.2; // minnaert roughness  1.5 default (1.0 is lambert)
	vec4 Idiff = vec4(gl_FrontMaterial.diffuse.rgb * minnaert(L, n, roughness), 1);
	Idiff = clamp(Idiff, 0.0, 1.0);

	//--- Physically based shader for specular lighting (energy conservation - normalization)
	//gl_FrontMaterial.shininess = 128;
	const float PI = 3.141592653;

	float normalisation_term = (gl_FrontMaterial.shininess + 2.0f) / 2.0f * PI;
	float blinn_phong = pow(dot(R, E), gl_FrontMaterial.shininess);    // n_dot_h is the saturated dot product of the normal and half vectors
	...
	float alpha = 1.0f / (sqrt((PI / 4.0f) * gl_FrontMaterial.shininess + (PI / 2.0f)));
	...
	vec3 myspecular = (PI / 4.0f) * specular_term * cosine_term * fresnel_term * visibility_term * gl_LightSource[0].specular;
	...
	final_color = vec4(myspecular + diffuseTranslucency + forwardTranslucency + vec3(Idiff) + vec3(Iamb), gl_Color.a);
}
...
void main()
{
	...
	{
		//if (fract(vertexID) < 0.001){
		//	gl_FragColor = gl_Color + 0.01; 
		//			return;
		//}
